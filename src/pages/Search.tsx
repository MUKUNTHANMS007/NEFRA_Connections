    ...
    <div className="w-full">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 flex items-center gap-2">
          <div className="h-px w-8 bg-white/10" /> Database Query
        </p>
        ...
        <div className="relative mt-10">
          <div className="relative flex items-center">
            <SearchIcon className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-indigo-300/80" />
            <input
              type="search"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search by name, role, location, or industry..."
              className="w-full rounded-2xl border border-white/10 bg-slate-900/50 py-5 pl-14 pr-4 text-slate-100 shadow-xl transition-colors placeholder:text-slate-500 focus:border-indigo-500/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/10"
            />
          </div>
        </div>
            ...
            <button
              ...
              className={`rounded-xl px-5 py-2.5 text-sm font-bold tracking-wide transition-all duration-300 ${
                activeFilter === f
                  ...
                  : 'border border-white/10 bg-slate-900/40 text-slate-400 hover:border-white/20 hover:text-slate-200 hover:bg-slate-900/60'
              }`}
            >
              {f}
            </button>
        ...
        <div className="mt-12 flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <p className="font-mono text-sm font-bold text-slate-300">[{filtered.length}] NODES_FOUND</p>
        </div>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((u) => (
            <Link
              key={String(u.id)}
              to={`/profile/${u.id}`}
              className="group relative flex flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/40 p-6 transition-all duration-500 hover:-translate-y-1 hover:border-white/20 hover:bg-slate-900/60 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.35)]"
            >
              <div className="relative z-10 flex items-start gap-5">
                <img
                  src={avatarUrl(u)}
                  alt="Profile Avatar"
                  className="h-14 w-14 shrink-0 rounded-2xl object-cover ring-1 ring-white/10 transition-all duration-500 group-hover:ring-white/20"
                />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">{u.role ?? 'Member'}</p>
        ...
        {filtered.length === 0 && !error && (
          <div className="mt-12 rounded-[2.5rem] border border-white/10 bg-slate-900/30 p-16 text-center">
            <p className="font-mono text-slate-400">0_RESULTS. ADJUST_PARAMETERS.</p>
          </div>
        )}

        {/* CTA */}
        <div className="mt-20 rounded-[2.5rem] border border-white/10 bg-slate-900/30 p-12 text-center shadow-2xl relative overflow-hidden">
          ...
          <Link
            to="/profile"
            className="relative z-10 mt-8 inline-flex items-center gap-2 rounded-xl bg-white/10 border border-white/10 px-8 py-3.5 font-black uppercase tracking-widest text-white shadow-lg transition-all duration-200 hover:bg-white/20"
          >
            Optimize Profile
          </Link>
        </div>
